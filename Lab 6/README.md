...
The next step is to deploy the Instana agent. Prior to installing the agent, request a trial of Instana at https://www.instana.com/.
...
Download the instana-agent.yml file, located  here: http://www.instana.com/media/instana-agent.yml_.txt and make two edits within the file. Note that the YAML (.yml) files are very syntax sensitive.
